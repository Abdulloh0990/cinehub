...
async function ytsFetch(path) {
  ...
  const urls = [
    ...YTS_BASES.map(b => b + path),
    ...YTS_BASES.flatMap(b => CORS_PROXIES.map(p => p(b + path))),
  ];
  for (const url of urls) {
    try {
      ...
      if (d?.data?.movies) {
        apiCache.set(path, { data: d, ts: Date.now() });
        return d;
      }
    } catch {}
  }
...
═══════════════════════════════════════════════ */
const PLAYERS = [
  { id:'kodik',  n:'Kodik',    url: id => `https://kodik.info/search?q=${id}&d=kodik.info` },
  { id:'vidsrc', n:'VidSrc',   url: id => `https://vidsrc.pro/embed/movie/${id}` },
  { id:'movie2', n:'Movies2',  url: id => `https://moviesapi.club/movie/${id}` },
  { id:'vicu',   n:'VidSrc.icu', url: id => `https://vidsrc.icu/embed/movie/${id}` },
  { id:'videasy',n:'Videasy',  url: id => `https://player.videasy.net/movie/${id}` },
  { id:'embedsu',n:'Embed.su', url: id => `https://embed.su/embed/movie/${id}` },
];
...
const GENRES = [
  ...
  { id:'sci-fi',       e:'🚀', uz:"Ilmiy",      ru:'Фантастика',    en:'Sci-Fi'      },
...
const T = {
  ...
  episodes: { uz:'qism',        ru:'серий',      en:'ep'        },
...
/* ═══════════════════════════════════════════════
   INTERSECTION OBSERVER HOOK (lazy image load)
═══════════════════════════════════════════════ */
const useInView = (ref, rootMargin = '300px') => {
  const [inView, setInView] = useState(false);
  useEffect(() => {
    if (!ref.current) return;
    // Already visible (cached)
    if (imgCache.has(ref.current?.dataset?.src)) { setInView(true); return; }
...
@media(min-width:1024px){.modal-wrap{flex-direction:row!important}}
...
/* ═══════════════════════════════════════════════
   3D ICON COMPONENTS
═══════════════════════════════════════════════ */
const Icon3D = memo(({ type, size = 32, color = '#fff', glow }) => {
  const icons = {
    home: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs>
          <linearGradient id="h1" x1="0" y1="0" x2="1" y2="1"><stop stopColor={color} stopOpacity=".9"/><stop offset="1" stopColor={color} stopOpacity=".4"/></linearGradient>
          <linearGradient id="h2" x1="0" y1="0" x2="0" y2="1"><stop stopColor={color} stopOpacity=".7"/><stop offset="1" stopColor={color} stopOpacity=".2"/></linearGradient>
        </defs>
        <path d="M16 4L3 14v15h9v-8h8v8h9V14L16 4z" fill="url(#h1)"/>
        <path d="M16 4L3 14h26L16 4z" fill="url(#h2)" opacity=".7"/>
        <rect x="13" y="21" width="6" height="8" fill={color} opacity=".5" rx="1"/>
      </svg>
    ),
    film: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs><linearGradient id="f1" x1="0" y1="0" x2="1" y2="1"><stop stopColor={color}/><stop offset="1" stopColor={color} stopOpacity=".3"/></linearGradient></defs>
        <rect x="3" y="7" width="26" height="18" rx="3" fill="url(#f1)" opacity=".85"/>
        <rect x="3" y="7" width="5" height="18" fill={color} opacity=".3" rx="2"/>
        <rect x="24" y="7" width="5" height="18" fill={color} opacity=".3" rx="2"/>
        {[0,1,2,3].map(i=><rect key={i} x="4" y={9+i*4} width="3" height="2" fill="rgba(0,0,0,.6)" rx="1"/>)}
        {[0,1,2,3].map(i=><rect key={i} x="25" y={9+i*4} width="3" height="2" fill="rgba(0,0,0,.6)" rx="1"/>)}
        <polygon points="14,13 14,19 20,16" fill="rgba(0,0,0,.5)"/>
      </svg>
    ),
    star: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs><linearGradient id="s1" x1="0" y1="0" x2="1" y2="1"><stop stopColor={color}/><stop offset="1" stopColor={color} stopOpacity=".5"/></linearGradient></defs>
        <polygon points="16,3 19.5,12.5 30,12.5 21.5,18.5 24.5,29 16,23 7.5,29 10.5,18.5 2,12.5 12.5,12.5" fill="url(#s1)"/>
        <polygon points="16,3 19.5,12.5 16,10" fill={color} opacity=".4"/>
      </svg>
    ),
    heart: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs><linearGradient id="hrt1" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#f43f5e"/><stop offset="1" stopColor="#e11d48" stopOpacity=".6"/></linearGradient></defs>
        <path d="M16 28s-13-8-13-17a8 8 0 0 1 13-6.2A8 8 0 0 1 29 11c0 9-13 17-13 17z" fill="url(#hrt1)"/>
      </svg>
    ),
    bookmark: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs><linearGradient id="bm1" x1="0" y1="0" x2="1" y2="1"><stop stopColor={color}/><stop offset="1" stopColor={color} stopOpacity=".4"/></linearGradient></defs>
        <path d="M8 4h16a2 2 0 0 1 2 2v22l-10-6-10 6V6a2 2 0 0 1 2-2z" fill="url(#bm1)"/>
        <path d="M8 4h4v6l-2-1.5L8 10V4z" fill={color} opacity=".5"/>
      </svg>
    ),
    grid: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs><linearGradient id="g1" x1="0" y1="0" x2="1" y2="1"><stop stopColor={color}/><stop offset="1" stopColor={color} stopOpacity=".4"/></linearGradient></defs>
        {[[3,3],[17,3],[3,17],[17,17]].map(([x,y],i)=><rect key={i} x={x} y={y} width="12" height="12" rx="3" fill="url(#g1)" opacity={[1,.8,.8,.6][i]}/>)}
      </svg>
    ),
    clock: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs><linearGradient id="c1" x1="0" y1="0" x2="1" y2="1"><stop stopColor={color}/><stop offset="1" stopColor={color} stopOpacity=".4"/></linearGradient></defs>
        <circle cx="16" cy="16" r="13" fill="url(#c1)" opacity=".85"/>
        <circle cx="16" cy="16" r="11" fill="none" stroke={color} strokeOpacity=".2" strokeWidth="1"/>
        <line x1="16" y1="9" x2="16" y2="16" stroke="rgba(0,0,0,.6)" strokeWidth="2" strokeLinecap="round"/>
        <line x1="16" y1="16" x2="21" y2="19" stroke="rgba(0,0,0,.5)" strokeWidth="2" strokeLinecap="round"/>
        <circle cx="16" cy="16" r="1.5" fill="rgba(0,0,0,.6)"/>
      </svg>
    ),
    chart: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs><linearGradient id="ch1" x1="0" y1="1" x2="0" y2="0"><stop stopColor={color} stopOpacity=".8"/><stop offset="1" stopColor={color}/></linearGradient></defs>
        {[[4,20,8],[12,12,8],[20,16,8],[24,6,4]].map(([x,y,h],i)=><rect key={i} x={x} y={y} width={i<3?6:4} height={32-y-4} rx="3" fill="url(#ch1)" opacity={[.5,.7,.6,1][i]}/>)}
        <line x1="3" y1="28" x2="29" y2="28" stroke={color} strokeOpacity=".3" strokeWidth="1.5" strokeLinecap="round"/>
      </svg>
    ),
    search: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs><linearGradient id="sr1" x1="0" y1="0" x2="1" y2="1"><stop stopColor={color}/><stop offset="1" stopColor={color} stopOpacity=".4"/></linearGradient></defs>
        <circle cx="14" cy="14" r="9" fill="url(#sr1)" opacity=".85"/>
        <circle cx="14" cy="14" r="7" fill="none" stroke="rgba(0,0,0,.3)" strokeWidth="1.5"/>
        <line x1="21" y1="21" x2="28" y2="28" stroke={color} strokeWidth="3" strokeLinecap="round"/>
      </svg>
    ),
    play: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <defs>
          <linearGradient id="pl1" x1="0" y1="0" x2="1" y2="1"><stop stopColor={color}/><stop offset="1" stopColor={color} stopOpacity=".6"/></linearGradient>
          <radialGradient id="pl2" cx="40%" cy="35%"><stop stopColor="rgba(255,255,255,.5)"/><stop offset="1" stopColor="transparent"/></radialGradient>
        </defs>
        <circle cx="16" cy="16" r="14" fill="url(#pl1)"/>
        <circle cx="16" cy="16" r="14" fill="url(#pl2)"/>
        <polygon points="13,11 13,21 22,16" fill="rgba(0,0,0,.55)"/>
      </svg>
    ),
    arrowLeft: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <path d="M20 6L10 16l10 10" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    ),
    arrowRight: (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        <path d="M12 6l10 10-10 10" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    ),
  };
  return (
    <span className="icon-3d" style={{ display:'inline-flex', filter:glow?`drop-shadow(0 0 10px ${color}99)`:undefined }}>
      {icons[type] ?? <span style={{ fontSize:size * 0.7 }}>▶</span>}
    </span>
  );
});
...
const StatusBadge = memo(({ status, lang }) => {
  ...
  const lb = { watching: t('watching', lang), planned: t('planned', lang), completed: t('completed', lang) };
...
/* ═══════════════════════════════════════════════
   PAGINATION COMPONENT (yaxshilangan)
═══════════════════════════════════════════════ */
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16, alignItems:'center', margin:'28px 0 8px' }}>
      ...
      <div style={{ display:'flex', gap:6, alignItems:'center', flexWrap:'wrap', justifyContent:'center' }}>
        ...
        {page > 3 && (
          <>
            ...
            {page > 4 && <span style={{ color:'rgba(255,255,255,.2)', fontSize:13, padding:'0 2px' }}>…</span>}
          </>
        )}
        ...
        {pages.map(pg => (
          <button
            key={pg}
            className={`pgn-btn${pg === page ? ' active' : ''}`}
            onClick={() => goTo(pg)}
            disabled={loading}
          >{pg}</button>
        ))}
        ...
        {hasMore && (
          <>
            <span style={{ color:'rgba(255,255,255,.2)', fontSize:13, padding:'0 2px' }}>…</span>
          </>
        )}
      </div>
      ...
      <div style={{ display:'flex', gap:10, alignItems:'center' }}>
        <button
          className="pgn-btn nav"
          onClick={() => goTo(page - 1)}
          disabled={page <= 1 || loading}
        >
          <Icon3D type="arrowLeft" size={16} color="rgba(255,255,255,.6)"/>
          {t('prevPage', lang)}
        </button>
        ...
        <div style={{ padding:'8px 18px', borderRadius:11, background:'rgba(255,255,255,.06)', border:'1px solid rgba(255,255,255,.1)', fontSize:12, fontWeight:700, color:'rgba(255,255,255,.4)', minWidth:80, textAlign:'center' }}>
          {loading ? <Spinner size={14} color="rgba(255,255,255,.4)"/> : <>{t('page', lang)} <span style={{ color:'white', fontWeight:800 }}>{page}</span></>}
        </div>
        ...
        >
          {t('nextPage', lang)}
          <Icon3D type="arrowRight" size={16} color={hasMore ? themeP : 'rgba(255,255,255,.3)'}/>
        </button>
      </div>

      {/* Total count */}
      {total > 0 && (
        <p style={{ fontSize:11, color:'rgba(255,255,255,.25)', fontWeight:600 }}>
          {t('page', lang)} {page} · {total} {t('films', lang)}
        </p>
      )}
    </div>
  );
...
const SetupScreen = memo(({ onSave }) => {
  ...
  const SQL = `-- CineHub SQL Setup (Supabase SQL Editor)
create table if not exists cinehub_data (
    ...
    <div style={{ position:'fixed', inset:0, background:'#050507', display:'flex', alignItems:'center', justifyContent:'center', padding:20, fontFamily:'DM Sans,system-ui,sans-serif', zIndex:99999, overflow:'auto' }}>
      <div style={{ width:'100%', maxWidth:500, display:'flex', flexDirection:'column', gap:20 }}>
        <div style={{ textAlign:'center' }}>
          <div style={{ width:72, height:72, borderRadius:22, background:'linear-gradient(135deg,#6366f1,#4f46e5)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px', boxShadow:'0 0 40px #6366f166', animation:'glow 3s ease infinite' }}>
            <span style={{ fontSize:30 }}>🎬</span>
          </div>
          <h1 className="syne" style={{ fontSize:30, fontWeight:800, color:'white', marginBottom:6 }}>CineHub Setup</h1>
          <p style={{ fontSize:13, color:'rgba(255,255,255,.35)' }}>Connect your Supabase backend for cloud sync</p>
        </div>
        <div style={{ display:'flex', gap:6, borderRadius:14, background:'rgba(255,255,255,.05)', padding:5 }}>
          {[['🗄 SQL Tables', 0],['🔑 API Config', 1]].map(([l, s]) => (
            <button key={s} onClick={() => setStep(s)} style={{ flex:1, padding:'10px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:'inherit', fontSize:12, fontWeight:700, transition:'all .2s', background:step===s?'linear-gradient(135deg,#6366f1,#4f46e5)':'transparent', color:step===s?'white':'rgba(255,255,255,.4)', boxShadow:step===s?'0 4px 14px #6366f144':'' }}>{l}</button>
          ))}
        </div>
        {step === 0 && (
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <div style={{ padding:16, borderRadius:14, background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.08)' }}>
              <p style={{ fontSize:12, fontWeight:600, color:'rgba(255,255,255,.7)', marginBottom:10 }}>Run this SQL in your Supabase project → SQL Editor:</p>
            ...
            <button onClick={() => setStep(1)} className="btn-p" style={{ padding:13, borderRadius:13, fontSize:14, boxShadow:'0 8px 24px #6366f144' }}>Next → API Config</button>
          </div>
        )}
        {step === 1 && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div>
              ...
              <input value={key} onChange={e => setKey(e.target.value)} placeholder="eyJhbGciOiJIUzI1NiIsInR5c..." style={{ ...inp, fontFamily:'monospace', fontSize:11 }}/>
            </div>
            <button onClick={handleSave} className="btn-p" style={{ padding:14, borderRadius:13, fontSize:14, boxShadow:'0 8px 28px #6366f144' }}>🚀 Launch CineHub</button>
          </div>
        )}
      </div>
    </div>
...
const AuthForm = memo(({ sb, themeP, onSuccess }) => {
  ...
  const doLogin = async () => {
    setErr(''); if (!email.includes('@') || pass.length < 6) { setErr('Check email and password (min 6 chars)'); return; }
    setLd(true);
    try {
      ...
      } else setErr(r?.error_description || r?.msg || 'Wrong email or password');
    } catch { setErr('Connection error'); } finally { setLd(false); }
  };

  const doRegister = async () => {
    setErr(''); if (!name.trim()) { setErr('Enter your name'); return; }
    if (!email.includes('@') || pass.length < 6) { setErr('Check email and password (min 6 chars)'); return; }
    setLd(true);
    try {
      ...
      if (r?.access_token) {
        ...
        if (r?.user?.id) {
          try {
            ...
            if (r2?.access_token) {
              const s = { access_token:r2.access_token, refresh_token:r2.refresh_token, user_id:r2.user?.id, email:r2.user?.email, expires_at:Date.now()+3600*1000 };
              LS.set(SESS_KEY, JSON.stringify(s)); onSuccess(r2); return;
            }
          } catch {}
          setErr('📧 Check your email to confirm, then sign in.');
        } else setErr(r?.error_description || r?.msg || 'Registration failed');
      }
    } catch { setErr('Connection error'); } finally { setLd(false); }
  };
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:13 }}>
      ...
      <button onClick={mode==='login'?doLogin:doRegister} className="btn-p" disabled={loading} style={{ padding:14, borderRadius:13, fontSize:14, boxShadow:`0 8px 24px ${themeP}44`, opacity:loading?.7:1 }}>
...
const SearchOverlay = memo(({ onClose, themeP, onOpen, lang }) => {
  ...
  useEffect(() => {
    ...
    debRef.current = setTimeout(async () => {
      try {
        const d = await ytsFetch(`/list_movies.json?query_term=${encodeURIComponent(q)}&limit=12&sort_by=rating`);
        setRes(d?.data?.movies ?? []);
      } catch {}
      setLd(false);
    }, 350);
  }, [q]);
      ...
      <div className="si" style={{ position:'relative', width:'100%', maxWidth:580 }}>
        <div className="glass-dark" style={{ borderRadius:22, overflow:'hidden', boxShadow:`0 32px 80px rgba(0,0,0,.8),0 0 0 1px ${themeP}33` }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, padding:'14px 16px', borderBottom:'1px solid rgba(255,255,255,.08)' }}>
            <Icon3D type="search" size={22} color={themeP}/>
            ...
            {ld ? <Spinner size={16} color={themeP}/> : q ? <button onClick={() => setQ('')} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,.4)', fontSize:16, padding:0 }}>✕</button> : null}
            <kbd style={{ fontSize:11, padding:'3px 7px', borderRadius:7, background:'rgba(255,255,255,.08)', color:'rgba(255,255,255,.3)', border:'1px solid rgba(255,255,255,.12)', flexShrink:0 }}>ESC</kbd>
          </div>
...
const FilterPanel = memo(({ filters, onChange, themeP, onClose, lang }) => {
  ...
  const QUALS = [{ v:'', l:'All' }, { v:'720p', l:'HD 720p' }, { v:'1080p', l:'FHD 1080p' }, { v:'2160p', l:'4K' }];
    ...
    <div className="fi" style={{ position:'fixed', inset:0, zIndex:11000, display:'flex', alignItems:'flex-end', justifyContent:'center', background:'rgba(0,0,0,.75)', backdropFilter:'blur(16px)' }}>
      ...
      <div className="glass-dark su page-sheet" style={{ position:'relative', width:'100%', padding:'18px 18px 36px', boxShadow:'0 -24px 70px rgba(0,0,0,.7)' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:16, alignItems:'center' }}>
          ...
          <button onClick={onClose} style={{ width:30, height:30, borderRadius:10, border:'none', background:'rgba(255,255,255,.08)', color:'rgba(255,255,255,.5)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'inherit' }}>✕</button>
        </div>
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          <div>
            <label style={{ display:'block', fontSize:11, fontWeight:700, opacity:.35, textTransform:'uppercase', letterSpacing:'.07em', marginBottom:8 }}>{t('genres', lang)}</label>
          ...
          <div>
            <label style={{ display:'block', fontSize:11, fontWeight:700, opacity:.35, textTransform:'uppercase', letterSpacing:'.07em', marginBottom:8 }}>Quality</label>
            <div style={{ display:'flex', gap:6 }}>
              {QUALS.map(q => (
                <button key={q.v} onClick={() => setLoc(p => ({ ...p, quality:q.v }))} style={{ flex:1, padding:'8px', borderRadius:10, border:`1px solid ${loc.quality===q.v?themeP+'66':'rgba(255,255,255,.1)'}`, background:loc.quality===q.v?`${themeP}22`:'rgba(255,255,255,.05)', color:loc.quality===q.v?themeP:'rgba(255,255,255,.45)', cursor:'pointer', fontSize:11, fontWeight:700, fontFamily:'inherit', textAlign:'center' }}>{q.l}</button>
              ))}
            </div>
          </div>
...
/* ═══════════════════════════════════════════════
   LAZY IMAGE (IntersectionObserver based)
═══════════════════════════════════════════════ */
      ...
      {visible && (
        <img
          ...
          style={{
            ...
            animation: loaded ? 'imgFadeIn .4s ease' : 'none',
          }}
        />
      )}
      {!loaded && (
...
/* ═══════════════════════════════════════════════
   MOVIE CARD  (optimized, language-aware)
═══════════════════════════════════════════════ */
const MovieCard = memo(({ movie, onClick, onFav, faved, status, userRating, themeP, lang = 'en' }) => {
  ...
  // Localized genre labels from our GENRES table
  ...
  return (
    <article
      ...
      style={{ cursor:'pointer', position:'relative', borderRadius:16, zIndex:hover?10:1 }}
    >
      <div className="movie-card" style={{ boxShadow:hover?`0 24px 48px rgba(0,0,0,.7),0 0 0 1px ${themeP}44`:'' }}>
        ...
        {hover && (
          <div style={{ position:'absolute', inset:0, background:`linear-gradient(to top,${themeP}44,transparent)` }}/>
        )}
        ...
        <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'8px 10px 10px' }}>
          ...
          <div style={{ display:'flex', gap:5, marginTop:3, flexWrap:'wrap' }}>
            {movie.year && (
              <span style={{ fontSize:10, fontWeight:600, color:'rgba(255,255,255,.4)' }}>{movie.year}</span>
            )}
            {movie.runtime > 0 && (
              <span style={{ fontSize:10, fontWeight:600, color:'rgba(255,255,255,.3)' }}>
                {movie.runtime} {minLbl}
              </span>
            )}
          </div>
          {/* Genre tags in selected language */}
          {genreLabels.length > 0 && (
            <div style={{ display:'flex', gap:4, marginTop:4, flexWrap:'wrap' }}>
              {genreLabels.map(g => (
                <span key={g} style={{ fontSize:9, padding:'1px 6px', borderRadius:20, background:`${themeP}22`, color:`${themeP}cc`, border:`1px solid ${themeP}25`, fontWeight:700 }}>{g}</span>
              ))}
            </div>
          )}
        </div>
        ...
        <button
          ...
          style={{ position:'absolute', bottom:9, right:9, width:30, height:30, borderRadius:9, border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, background:faved?'#f43f5e99':'rgba(0,0,0,.55)', backdropFilter:'blur(8px)', transition:'all .2s', color:'white', zIndex:2, animation:faved?'heartPop .3s ease':'' }}
        >
          {faved ? '♥' : '♡'}
        </button>
...
const DetailPanel = memo(({ movie, lib, ratings, notes, favs, wl, themeP, isAuth, onStatus, onRate, onNote, onFav, onWL, onShare, lang }) => {
  ...
  return (
    <div className="ns" style={{ overflowY:'auto', padding:14, display:'flex', flexDirection:'column', gap:11, height:'100%' }}>
      ...
      <div>
        <p style={{ fontSize:10, fontWeight:700, opacity:.35, textTransform:'uppercase', letterSpacing:'.07em', marginBottom:6 }}>{t('watching', lang)}</p>
...
/* ═══════════════════════════════════════════════
   EMBED PLAYER  (iframe-based, 6 sources)
═══════════════════════════════════════════════ */
const EmbedPlayer = memo(({ movie, themeP, lang, playerSrc, onPlayerChange }) => {
  ...
  const imdbId = movie?.imdb_code;  // e.g. "tt1375666"

  // Labels per language
  const WATCH_LBL  = { uz:'Ko\'rish',      ru:'Смотреть',  en:'Watch Now'   };
  const SOURCE_LBL = { uz:'Manba',         ru:'Источник',  en:'Source'      };
  const LOAD_LBL   = { uz:'Yuklanmoqda…',  ru:'Загрузка…', en:'Loading…'    };
  const ERR_LBL    = { uz:'IMDB kodi topilmadi', ru:'IMDB ID не найден', en:'No IMDB ID found' };

  const currentPlayer = PLAYERS.find(p => p.id === playerSrc) ?? PLAYERS[0];
  const embedUrl = imdbId ? currentPlayer.url(imdbId) : null;

  const handleStart = () => { setStarted(true); setLoading(true); };
  ...
  if (!imdbId) return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', gap:12, padding:24, textAlign:'center', background:'#000' }}>
      ...
      <p style={{ fontSize:14, color:'rgba(255,255,255,.4)', fontWeight:600 }}>{ERR_LBL[lang] ?? ERR_LBL.en}</p>
    </div>
  );
  ...
  return (
    <div style={{ width:'100%', height:'100%', display:'flex', flexDirection:'column', background:'#000' }}>
      {/* ── Player selector bar ── */}
      <div style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 10px', background:'rgba(0,0,0,.85)', flexShrink:0, borderBottom:'1px solid rgba(255,255,255,.08)', flexWrap:'wrap' }}>
        <span style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,.3)', textTransform:'uppercase', letterSpacing:'.06em', marginRight:4 }}>
          {SOURCE_LBL[lang] ?? SOURCE_LBL.en}:
        </span>
        {PLAYERS.map((pl, idx) => {
          ...
          return (
            <button
              key={pl.id}
              onClick={() => switchPlayer(pl.id)}
              style={{
                padding:'4px 12px', borderRadius:99, fontFamily:'inherit',
                border:`1px solid ${active ? themeP+'77' : 'rgba(255,255,255,.14)'}`,
                background: active ? `linear-gradient(135deg,${themeP},${themeP}88)` : 'rgba(255,255,255,.07)',
                color: active ? '#000' : 'rgba(255,255,255,.55)',
                cursor:'pointer', fontSize:11, fontWeight:800,
                transition:'all .18s',
                boxShadow: active ? `0 3px 12px ${themeP}55` : 'none',
              }}
            >
              {pl.n}
            </button>
      ...
      <div style={{ flex:1, position:'relative', minHeight:0, background:'#000' }}>
        {/* Poster / Play button (before start) */}
        {!started && (
          <div
            onClick={handleStart}
            style={{ position:'absolute', inset:0, cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:20, zIndex:2 }}
          >
            {/* Blurred poster bg */}
            {movie.medium_cover_image && (
              <img
                src={movie.medium_cover_image}
                alt=""
                style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover', filter:'blur(18px) brightness(.35) saturate(1.4)', transform:'scale(1.08)' }}
              />
            )}
            {/* Gradient overlay */}
            ...
            <div style={{ position:'relative', zIndex:3, textAlign:'center', padding:'0 24px' }}>
              <p className="syne lc2" style={{ fontSize:20, fontWeight:800, color:'white', marginBottom:8, lineHeight:1.3, textShadow:'0 2px 12px rgba(0,0,0,.8)' }}>{movie.title}</p>
              {movie.year && <span style={{ fontSize:13, color:'rgba(255,255,255,.55)', fontWeight:600 }}>{movie.year}</span>}
            </div>
            {/* Big play button */}
            <div
              style={{
                ...
                background:`linear-gradient(135deg,${themeP},${themeP}88)`,
                display:'flex', alignItems:'center', justifyContent:'center',
                boxShadow:`0 0 0 12px ${themeP}22, 0 0 50px ${themeP}66`,
            ...
            >
              <svg width="30" height="30" viewBox="0 0 32 32" fill="none">
                <polygon points="11,8 11,24 26,16" fill="rgba(0,0,0,.75)"/>
              </svg>
            </div>
            <p style={{ position:'relative', zIndex:3, fontSize:13, fontWeight:700, color:'rgba(255,255,255,.7)', letterSpacing:'.04em' }}>
              {WATCH_LBL[lang] ?? WATCH_LBL.en}
            </p>
          </div>
        )}
        ...
        {started && (
          <>
            {loading && (
              <div style={{ position:'absolute', inset:0, zIndex:3, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:14, background:'rgba(0,0,0,.9)' }}>
                <Spinner size={40} color={themeP}/>
                <p style={{ fontSize:13, fontWeight:700, color:'rgba(255,255,255,.55)' }}>{LOAD_LBL[lang] ?? LOAD_LBL.en}</p>
                <p style={{ fontSize:11, color:'rgba(255,255,255,.25)' }}>{currentPlayer.n}</p>
              </div>
            )}
            <iframe
              key={`${iframeKey}-${playerSrc}`}
              src={embedUrl}
              allowFullScreen
              ...
              style={{
                ...
                opacity: loading ? 0 : 1, transition:'opacity .4s',
              }}
            />
          </>
        )}
...
const ItemModal = memo(({ movie, ...rest }) => {
  ...
  return (
    <div className="fi" style={{ position:'fixed', inset:0, zIndex:9800, background:'rgba(0,0,0,.98)', display:'flex', flexDirection:'column' }}>
      <div className="glass-dark" style={{ display:'flex', alignItems:'center', gap:10, padding:'0 13px', height:50, flexShrink:0, borderBottom:'1px solid rgba(255,255,255,.07)' }}>
        ...
        {movie.year && <span style={{ fontSize:12, color:'rgba(255,255,255,.3)', fontWeight:600 }}>{movie.year}</span>}
        {movie.imdb_code && (
          <a href={`https://www.imdb.com/title/${movie.imdb_code}`} target="_blank" rel="noreferrer"
            style={{ fontSize:11, padding:'3px 9px', borderRadius:8, background:'rgba(234,179,8,.15)', color:'#fbbf24', border:'1px solid rgba(234,179,8,.3)', fontWeight:700, textDecoration:'none' }}>
            IMDb
          </a>
        )}
        <button onClick={onClose} style={{ width:32, height:32, borderRadius:10, border:'1px solid rgba(239,68,68,.35)', background:'rgba(239,68,68,.1)', cursor:'pointer', color:'#f87171', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>✕</button>
      </div>
      <div style={{ flex:1, display:'flex', overflow:'hidden' }} className="modal-wrap">
        <div style={{ background:'#000', flexShrink:0, minHeight:260, maxHeight:'52vh', position:'relative', flex:'none', width:'100%', overflow:'hidden' }} className="player-pane">
          <EmbedPlayer
            movie={movie}
            themeP={themeP}
            lang={lang}
      ...
      <style>{`@media(min-width:1024px){.player-pane{flex:1!important;max-height:none!important;width:auto!important;min-height:0!important}}`}</style>
    </div>
  );
});
  ...
  return (
    <section style={{ marginBottom:28 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12, paddingRight:4 }}>
        ...
        <div style={{ display:'flex', gap:6 }}>
          <button onClick={() => scroll(-1)} style={{ width:30, height:30, borderRadius:9, border:'1px solid rgba(255,255,255,.1)', background:'rgba(255,255,255,.06)', color:'rgba(255,255,255,.5)', cursor:'pointer', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center' }}>‹</button>
          <button onClick={() => scroll(1)} style={{ width:30, height:30, borderRadius:9, border:'1px solid rgba(255,255,255,.1)', background:'rgba(255,255,255,.06)', color:'rgba(255,255,255,.5)', cursor:'pointer', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center' }}>›</button>
        </div>
      </div>
      ...
      <div style={{ borderRadius:20, padding:20, background:`linear-gradient(135deg,${themeP}18,${themeS}0d)`, border:`1px solid ${themeP}33`, position:'relative', overflow:'hidden' }}>
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <p style={{ fontSize:10, color:'rgba(255,255,255,.3)', marginTop:4 }}>{stats.xpInLvl}/100 XP</p>
          </div>
        </div>
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:8 }}>
        {[[t('watched', lang), stats.watched,'#22c55e','✅'],[t('watching', lang), stats.watching,'#60a5fa','👁'],[t('planned', lang), stats.planned,'#f59e0b','⏳'],['Hours', stats.hours,'#a855f7','⏱'],['Rated', stats.rated,'#eab308','★'],['Avg Score', stats.avgRating,'#ec4899','♥'],['Favorites', favs,'#f43f5e','❤'],['Notes', stats.notes,'#14b8a6','📝']].map(([l, v, c, i]) => (
      ...
      <div className="card" style={{ borderRadius:18, padding:18 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>
          {ACHS.map(a => {
            ...
            return (
              <div key={a.id} title={a.n[lang] || a.n.en} style={{ borderRadius:12, padding:'9px 4px', textAlign:'center', background:done?`${themeP}18`:'rgba(255,255,255,.04)', border:`1px solid ${done?themeP+'44':'rgba(255,255,255,.06)'}`, opacity:done?1:.35, transition:'all .3s', animation:done?'pop .5s ease both':'' }}>
...
/* ═══════════════════════════════════════════════
   PROFILE SHEET
═══════════════════════════════════════════════ */
const ProfileSheet = memo(({ user, stats, achs, themeP, themeS, syncSt, curTheme, lang, sbCfg, onClose, onSave, onLogout, onExport, onImport, onUser, onTheme, onLang, onResetCfg, fileRef }) => {
...
export default function App() {
  ...
  /* ── Boot ── */
  useEffect(() => { const ti = setTimeout(() => setBoot(false), 1600); return () => clearTimeout(ti); }, []);
    ...
    (async () => {
      try {
        const [d1, d2, d3] = await Promise.all([
          ytsFetch('/list_movies.json?limit=24&sort_by=download_count&minimum_rating=6'),
          ytsFetch('/list_movies.json?limit=24&sort_by=date_added'),
          ytsFetch('/list_movies.json?limit=24&sort_by=rating&minimum_rating=7.5'),
        ]);
        const p = d1?.data?.movies ?? [];
        const l = d2?.data?.movies ?? [];
        const tr = d3?.data?.movies ?? [];
        setPopular(p); setLatest(l); setTopRated(tr);
        setHomeLoaded(true);
        // Preload first row of images
        p.slice(0, 8).forEach(m => preloadImg(m.medium_cover_image));
        tr.slice(0, 4).forEach(m => preloadImg(m.large_cover_image || m.background_image_original));
      } catch {}
    })();
  }, [homeLoaded]);

  /* ── Browse fetch + prefetch next page ── */
  const browsePathFor = useCallback((pg) => {
    let path = `/list_movies.json?limit=${PER_PAGE}&page=${pg}&sort_by=${sort}`;
    if (filters.genre) path += `&genre=${filters.genre}`;
    if (filters.year) path += `&query_term=${filters.year}`;
    if (filters.ratingMin > 0) path += `&minimum_rating=${filters.ratingMin}`;
    if (filters.quality) path += `&quality=${filters.quality}`;
    return path;
  }, [sort, filters]);
  ...
  useEffect(() => {
    ...
    setLoadBr(true);
    ytsFetch(browsePathFor(page)).then(d => {
      const movies = d?.data?.movies ?? [];
      const total = d?.data?.movie_count ?? 0;
      setBrowse(movies);
      setBrowseTotal(total);
      setLoadBr(false);
      // Preload visible images
      movies.slice(0, 12).forEach(m => preloadImg(m.medium_cover_image));
      // Prefetch next page into cache silently
      if (movies.length === PER_PAGE) {
        setTimeout(() => ytsFetch(browsePathFor(page + 1)), 800);
      }
    }).catch(() => setLoadBr(false));
  }, [view, page, browsePathFor]);
  ...
  const onAuthSuccess = useCallback(async (r) => {
    ...
    const nm = r.user?.user_metadata?.display_name || r.user?.email?.split('@')[0] || 'Cinefilm Fan';
    try {
    ...
    } catch { toast('Data loaded locally', 'info'); }
  ...
  const handleStatus = useCallback((movie, status) => {
    ...
    const labels = { watching:t('watching', lang), planned:t('planned', lang), completed:t('watched', lang) };
    toast(labels[status]);
  ...
  const hasFilters = filters.genre || filters.year || filters.ratingMin > 0 || filters.quality;
  ...
  const displayData = useMemo(() => {
    if (view === 'library') return Object.values(library);
    if (view === 'favs') return favs;
    if (view === 'watchlist') return wl;
    if (view === 'history') return histD;
    if (view === 'browse') return browse;
    return [];
  }, [view, library, favs, wl, histD, browse]);
  ...
  if (boot) return (
    <div style={{ position:'fixed', inset:0, background:theme.b, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:28, fontFamily:'DM Sans,system-ui,sans-serif' }}>
      ...
      <div style={{ display:'flex', gap:10 }}>
        {[0,1,2,3,4].map(i => <div key={i} style={{ width:8, height:8, borderRadius:'50%', background:theme.p, animation:`float .9s ${i * .13}s ease-in-out infinite` }}/>)}
      </div>
    </div>
  );
  ...
  /* ════════════ NAV ITEMS ════════════ */
  const NAV = [
    { k:'home',      l:t('home',lang),       i:'home'    },
    { k:'browse',    l:t('browse',lang),      i:'film'    },
    { k:'favs',      l:t('favs',lang),        i:'heart'   },
    { k:'watchlist', l:t('watchlist',lang),   i:'bookmark'},
    { k:'library',   l:t('library',lang),     i:'grid'    },
    { k:'history',   l:t('history',lang),     i:'clock'   },
    { k:'stats',     l:t('stats',lang),       i:'chart'   },
  ];
  ...
  return (
    <div style={{ minHeight:'100dvh', background:theme.b, overflowX:'hidden', position:'relative' }}>
      ...
      {profOpen && isAuth && (
        <ProfileSheet user={user} stats={stats} achs={achs} themeP={theme.p} themeS={theme.s} syncSt={syncSt} curTheme={curTheme} lang={lang} sbCfg={sbCfg}
          ...
          <div className="glass-dark su page-sheet" style={{ position:'relative', padding:'20px 20px 38px', boxShadow:'0 -24px 70px rgba(0,0,0,.75)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:13, marginBottom:20 }}>
              ...
              <div>
                ...
                <p style={{ fontSize:12, opacity:.35 }}>Sync your watchlist across devices</p>
              </div>
      ...
      {logoutDlg && (
        <div className="fi" style={{ position:'fixed', inset:0, zIndex:9600, display:'flex', alignItems:'center', justifyContent:'center', padding:'0 20px', background:'rgba(0,0,0,.88)', backdropFilter:'blur(18px)' }}>
          <div className="glass-dark si" style={{ borderRadius:22, padding:'28px 24px', textAlign:'center', maxWidth:300, width:'100%' }}>
            ...
            <p style={{ fontSize:12, opacity:.35, marginBottom:22 }}>Your data is saved to Supabase.</p>
            <div style={{ display:'flex', gap:10 }}>
      ...
      <header style={{ position:'sticky', top:0, zIndex:700, background:`${theme.b}ee`, backdropFilter:'blur(28px) saturate(200%)', borderBottom:'1px solid rgba(255,255,255,.07)' }}>
        <div style={{ maxWidth:1800, margin:'0 auto', padding:'0 16px', height:58, display:'flex', alignItems:'center', gap:10 }}>
          <div onClick={() => { setView('home'); setFilters({ genre:'', year:null, ratingMin:0, quality:'' }); setPage(1); }} style={{ display:'flex', alignItems:'center', gap:9, cursor:'pointer', flexShrink:0, userSelect:'none' }}>
            <div style={{ width:36, height:36, borderRadius:11, background:`linear-gradient(135deg,${theme.p},${theme.s})`, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:`0 0 18px ${theme.p}55`, animation:'glow 4s ease infinite', flexShrink:0, transition:'transform .3s' }}
              onMouseEnter={e => e.currentTarget.style.transform='scale(1.1) rotate(-5deg)'}
              onMouseLeave={e => e.currentTarget.style.transform='scale(1) rotate(0)'}>
              <span style={{ fontSize:18 }}>🎬</span>
            </div>
            <span className="syne" style={{ fontSize:20, fontWeight:800, background:`linear-gradient(135deg,${theme.p},#fff)`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', display:'none' }} id="logo-text">CineHub</span>
          </div>

          <nav className="desk" style={{ gap:2 }}>
            {NAV.map(v => (
              <button key={v.k} onClick={() => { setView(v.k); if (v.k === 'browse') setPage(1); }} style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 11px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:'inherit', fontSize:12, fontWeight:700, transition:'all .2s', background:view===v.k?`${theme.p}1e`:'transparent', color:view===v.k?theme.p:'rgba(255,255,255,.38)', borderBottom:view===v.k?`2px solid ${theme.p}`:'2px solid transparent' }}>
                <Icon3D type={v.i} size={16} color={view===v.k?theme.p:'rgba(255,255,255,.4)'}/>
                {v.l}
              </button>
            ))}
          </nav>
          ...
          <div onClick={() => setSearchOpen(true)} style={{ flex:1, maxWidth:420, margin:'0 10px', cursor:'pointer' }}>
            <div style={{ display:'flex', alignItems:'center', gap:9, padding:'9px 14px', borderRadius:12, background:'rgba(255,255,255,.07)', border:'1.5px solid rgba(255,255,255,.09)', transition:'all .2s' }}
              ...
              <Icon3D type="search" size={17} color="rgba(255,255,255,.3)"/>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:7, flexShrink:0, marginLeft:'auto' }}>
            <button onClick={() => setFilterOpen(true)} style={{ width:36, height:36, borderRadius:11, border:`1px solid ${hasFilters?theme.p+'55':'rgba(255,255,255,.1)'}`, background:hasFilters?`${theme.p}18`:'rgba(255,255,255,.07)', cursor:'pointer', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s', color:hasFilters?theme.p:'rgba(255,255,255,.45)', position:'relative' }}>
              ⚙
              {hasFilters && <div style={{ position:'absolute', top:4, right:4, width:6, height:6, borderRadius:'50%', background:theme.p }}/>}
            </button>
            <button onClick={randomMovie} className="desk" style={{ width:36, height:36, borderRadius:11, border:'1px solid rgba(255,255,255,.1)', background:'rgba(255,255,255,.07)', cursor:'pointer', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s' }}
              title="Random movie"
              onMouseEnter={e => { e.currentTarget.style.background='rgba(255,255,255,.14)'; e.currentTarget.style.transform='rotate(20deg)'; }}
              onMouseLeave={e => { e.currentTarget.style.background='rgba(255,255,255,.07)'; e.currentTarget.style.transform='rotate(0)'; }}>🎲</button>
            ...
            {!isAuth ? (
              ...
              <button onClick={() => setProfOpen(true)} style={{ background:'none', border:'none', padding:0, cursor:'pointer' }}>
                <img src={user.avatar} alt="" style={{ width:36, height:36, borderRadius:11, objectFit:'cover', border:`2px solid ${theme.p}`, boxShadow:`0 0 14px ${theme.p}55`, display:'block', transition:'transform .2s' }}
        ...
        <div className="ns mob" style={{ overflowX:'auto', gap:5, padding:'5px 14px 8px', borderTop:'1px solid rgba(255,255,255,.06)' }}>
          {NAV.map(v => (
            <button key={v.k} onClick={() => setView(v.k)} style={{ flexShrink:0, display:'flex', alignItems:'center', gap:5, padding:'6px 12px', borderRadius:99, border:'none', cursor:'pointer', fontFamily:'inherit', fontSize:12, fontWeight:700, whiteSpace:'nowrap', transition:'all .2s', background:view===v.k?`linear-gradient(135deg,${theme.p},${theme.s})`:'rgba(255,255,255,.07)', color:view===v.k?'#000':'rgba(255,255,255,.38)', boxShadow:view===v.k?`0 4px 12px ${theme.p}44`:'' }}>
              <Icon3D type={v.i} size={14} color={view===v.k?'#000':'rgba(255,255,255,.4)'}/>
              {v.l}
            </button>
          ))}
        </div>
      ...
      <div style={{ maxWidth:1800, margin:'0 auto', padding:'12px 16px', display:'flex', gap:14, alignItems:'flex-start', position:'relative', zIndex:2 }} className="main-area">
          ...
          <div className="card" style={{ borderRadius:20, padding:14, display:'flex', flexDirection:'column', gap:13 }}>
            <div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                {[['rating','★ Top Rated'],['download_count','🔥 Popular'],['date_added','📅 Newest'],['like_count','👍 Liked']].map(([v, l]) => (
        ...
        <main style={{ flex:1, minWidth:0 }}>
            ...
            <div className="fu">
              {topRated[0] && (
                <div onClick={() => openMovie(topRated[0])} style={{ borderRadius:24, overflow:'hidden', position:'relative', height:320, cursor:'pointer', marginBottom:28, boxShadow:`0 20px 60px rgba(0,0,0,.7),0 0 0 1px ${theme.p}22` }}
                  ...
                  <img className="hero-img" src={topRated[0].background_image_original || topRated[0].large_cover_image} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', objectPosition:'top', transition:'transform .6s ease', display:'block' }}/>
                  ...
                  <div style={{ position:'absolute', inset:0, padding:'28px 32px', display:'flex', flexDirection:'column', justifyContent:'flex-end' }}>
                    ...
                    <div style={{ display:'flex', gap:10, marginTop:14 }}>
                      <button className="btn-p" style={{ padding:'11px 22px', borderRadius:12, fontSize:13, display:'flex', alignItems:'center', gap:8, boxShadow:`0 8px 24px ${theme.p}55` }}>
                        <Icon3D type="play" size={18} color="#000"/>
                        {t('watch', lang)}
                      </button>
            ...
            <div className="fu">
              ...
              {loadBr ? (
                ...
                <div style={{ textAlign:'center', padding:'64px 20px' }}>
                  <span style={{ fontSize:52, animation:'float 3s ease infinite', display:'block', marginBottom:14 }}>📭</span>
                  ...
                  {browse.map((m, idx) => (
                    <div key={`${m.id}-${idx}`} className="card" style={{ borderRadius:14, display:'flex', gap:12, padding:'11px 12px', cursor:'pointer', transition:'background .2s' }}
                      ...
                      <div style={{ flex:1, minWidth:0, display:'flex', flexDirection:'column', justifyContent:'center', gap:5 }}>
                        ...
                        {m.summary && <p className="lc2" style={{ fontSize:11, color:'rgba(255,255,255,.32)', lineHeight:1.55 }}>{m.summary}</p>}
                      </div>
                      <button onClick={e => { e.stopPropagation(); handleFav(m); }} style={{ width:32, height:32, borderRadius:9, border:'none', cursor:'pointer', background:favIds.has(m.id)?'#f43f5e22':'rgba(255,255,255,.06)', color:favIds.has(m.id)?'#f43f5e':'rgba(255,255,255,.3)', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center', alignSelf:'center' }}>
                        {favIds.has(m.id) ? '♥' : '♡'}
                      </button>
                    </div>
                  ))}
          ...
          {['library', 'favs', 'watchlist', 'history'].includes(view) && (
            <div className="fu">
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
                <div>
                  <h1 className="syne" style={{ fontSize:20, fontWeight:800, marginBottom:2 }}>
                    {{ library:t('library',lang), favs:t('favs',lang), watchlist:t('watchlist',lang), history:t('history',lang) }[view]}
                  </h1>
              ...
              {!isAuth ? (
                <div style={{ textAlign:'center', padding:'64px 20px' }}>
                  ...
                  <button onClick={() => setAuthOpen(true)} className="btn-p" style={{ padding:'11px 24px', borderRadius:12, fontSize:14, boxShadow:`0 8px 22px ${theme.p}44` }}>{t('login', lang)}</button>
                </div>
      ...
      <nav className="mob" style={{ position:'fixed', bottom:0, left:0, right:0, zIndex:600, background:`${theme.b}f5`, backdropFilter:'blur(24px) saturate(200%)', borderTop:'1px solid rgba(255,255,255,.08)', paddingBottom:'env(safe-area-inset-bottom,0)', justifyContent:'center' }}>
        <div style={{ display:'flex', alignItems:'center', width:'100%', maxWidth:460, padding:'6px 8px' }}>
          {[
            { k:'home', i:'home', l:t('home',lang) },
            { k:'browse', i:'film', l:t('browse',lang) },
            { k:'favs', i:'heart', l:t('favs',lang) },
            { k:'library', i:'grid', l:t('library',lang) },
            { k:'stats', i:'chart', l:t('stats',lang) },
          ].map(v => (
            <button key={v.k} onClick={() => setView(v.k)} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3, padding:'6px 0', border:'none', borderRadius:13, cursor:'pointer', fontFamily:'inherit', background:view===v.k?`${theme.p}1e`:'transparent', transition:'all .2s' }}>
              <Icon3D type={v.i} size={20} color={view===v.k?theme.p:'rgba(255,255,255,.26)'}/>
              <span style={{ fontSize:9, fontWeight:700, color:view===v.k?theme.p:'rgba(255,255,255,.26)' }}>{v.l}</span>
            </button>
          ))}
          {isAuth ? (
            <button onClick={() => setProfOpen(true)} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3, border:'none', background:'transparent', cursor:'pointer', padding:'5px 0' }}>
            ...
            <button onClick={() => setAuthOpen(true)} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3, border:'none', background:`${theme.p}1e`, borderRadius:13, cursor:'pointer', padding:'6px 0' }}>
              <span style={{ fontSize:18 }}>👤</span>
      ...
      <footer className="desk" style={{ borderTop:'1px solid rgba(255,255,255,.06)', padding:'24px 18px', flexDirection:'column', gap:4, marginTop:16, textAlign:'center', position:'relative', zIndex:2 }}>
        ...
        <p style={{ fontSize:11, color:'rgba(255,255,255,.14)' }}>© 2026 · Powered by YTS.mx · Built with ❤</p>
      </footer>
